...
export default function StoryReader() {
  ...
  const { data: story, isLoading: loading, isError, error: queryError } = useQuery({
    ...
    queryFn: async () => {
      ...
      let res;
      if (isPreview) {
        res = await fetch('/api/admin/stories/preview', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ storyId })
        });
      } else {
        res = await fetch(`/api/stories/get?id=${storyId}`);
      }
  ...
  const renderContent = () => {
    ...
    return <div className="font-solway leading-[1.4] text-[0.95rem] whitespace-pre-wrap text-black break-words">{elements}</div>;
  };
  ...
  return (
    <div className={`w-full flex flex-col relative ${isLandscapeMode ? 'h-[100dvh] overflow-hidden' : 'min-h-[100dvh] overflow-x-hidden'}`} style={{ background: bg }}>
      ...
      {isLandscapeMode && (
        <div className="fixed flex flex-col gap-[8px] z-[1000]" style={{ top: '100px', right: '25px' }}>
      ...
      {isLandscapeMode ? (
        ...
        <div className="flex flex-col items-center justify-start w-full min-h-screen px-6 pt-[20px] pb-[120px] overflow-y-auto">
