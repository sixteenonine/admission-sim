...
const ForgotPassword = ({ onBack, themeVals }) => {
  ...
  const { theme, shadowDeepInset } = themeVals;
  ...
  return (
    <div className="w-full flex flex-col gap-5 relative">
      <style>{`
        ...
        .fade-transition { animation: pureFade 0.4s ease-out forwards; }
      `}</style>

      {step < 4 && (
        <button onClick={step === 1 ? onBack : () => setStep(step - 1)} className="flex items-center text-[14px] font-medium opacity-60 hover:opacity-100 transition-opacity w-fit -ml-2 p-2" style={{ color: theme.textMain }}>
      ...
      {step === 1 && (
        <form onSubmit={handleCheckUsername} className="flex flex-col gap-6 fade-transition">
          ...
          <button disabled={isStep1Disabled} className={`w-full mt-2 h-[54px] rounded-md font-bold text-[15px] transition-all flex items-center justify-center gap-2 ${isStep1Disabled ? 'opacity-50 cursor-not-allowed' : 'active:scale-[0.98]'}`} style={{ background: '#007bff', color: '#ffffff' }}>
      ...
      {step === 2 && (
        <form onSubmit={handleAnswerSubmit} className="flex flex-col gap-6 fade-transition">
          <div className="flex flex-col">
            ...
            <h3 className="text-[28px] font-light tracking-wide leading-tight" style={{ color: theme.textMain }}>{SECURITY_QUESTIONS[formData.securityQuestionId]}</h3>
          </div>
          ...
          <button disabled={isStep2Disabled} className={`w-full mt-2 h-[54px] rounded-md font-bold text-[15px] transition-all flex items-center justify-center gap-2 ${isStep2Disabled ? 'opacity-50 cursor-not-allowed' : 'active:scale-[0.98]'}`} style={{ background: '#007bff', color: '#ffffff' }}>
            {loading ? <Loader2 className="animate-spin" size={18} /> : null} Next
          </button>
        </form>
      )}
        ...
        <form onSubmit={handleResetPassword} className="flex flex-col gap-6 fade-transition">
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              ...
              <div className={`flex items-center px-4 h-[52px] rounded-md border transition-all ${isPassInvalid ? 'border-red-500' : 'border-gray-400/80 focus-within:border-[#007bff]'}`} style={{ background: '#d3d7da50' }}>
                <input type={showPassword ? "text" : "password"} required className="w-full bg-transparent outline-none text-[15px] font-medium" value={formData.newPassword} onChange={(e) => setFormData({...formData, newPassword: e.target.value})} style={{ color: theme.textMain }} />
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className={`flex items-center px-4 h-[52px] rounded-md border transition-all ${isConfirmInvalid ? 'border-red-500' : 'border-gray-400/80 focus-within:border-[#007bff]'}`} style={{ background: '#d3d7da50' }}>
                <input type={showConfirmPassword ? "text" : "password"} required className="w-full bg-transparent outline-none text-[15px] font-medium" value={formData.confirmNewPassword} onChange={(e) => setFormData({...formData, confirmNewPassword: e.target.value})} style={{ color: theme.textMain }} />
          ...
          <button disabled={isStep3Disabled} className={`w-full mt-2 h-[54px] rounded-md font-bold text-[15px] transition-all flex items-center justify-center gap-2 ${isStep3Disabled ? 'opacity-50 cursor-not-allowed' : 'active:scale-[0.98]'}`} style={{ background: '#007bff', color: '#ffffff' }}>
            {loading ? <Loader2 className="animate-spin" size={18} /> : null} Next
          </button>
        </form>
      )}
      ...
      {step === 4 && (
        <div className="flex flex-col items-center text-center py-8 fade-transition">
          ...
          <h2 className="text-[32px] font-light tracking-wide mb-4" style={{ color: theme.textMain }}>เรียบร้อย!</h2>
          <p className="text-[15px] font-medium opacity-80 mb-8" style={{ color: theme.textMain }}>รหัสผ่านถูกเปลี่ยนแล้ว สามารถเข้าสู่ระบบได้ทันที</p>
          <button onClick={onBack} className="w-full h-[54px] rounded-md font-bold text-[15px] transition-all active:scale-[0.98]" style={{ background: '#007bff', color: '#ffffff' }}>Next</button>
        </div>
      )}
    </div>
  );
