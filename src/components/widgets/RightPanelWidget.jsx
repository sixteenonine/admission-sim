          ...
          {isLcdEditOpen && (
             <div className="absolute top-1/2 -translate-y-1/2 left-[calc(100%+16px)] w-[260px] rounded-[1.5rem] p-2.5 border border-white/20 z-[100] transition-all flex flex-col gap-1" style={{ background: raisedGradient, boxShadow: shadowPlateau }}>
                ...
                <div className="w-full px-5 pt-2 pb-3 flex flex-col gap-3 border-t border-white/5">
                  ...
                  <input type="range" min="0" max="360" value={Math.round(trackHue)} onChange={(e) => { setTrackHue(Number(e.target.value)); setIsAutoTrackHue(false); }} onClick={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} onTouchMove={(e) => e.stopPropagation()} className="w-full h-1.5 rounded-full accent-[#3b82f6] bg-black/10 outline-none" style={{ boxShadow: shadowTrench }} />
                </div>
            </div>
          )}
        </div>
